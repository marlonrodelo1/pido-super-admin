    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 20 }}>
        <div style={ds.card}>
          ...
          <div style={{ fontSize: 28, fontWeight: 800, color: '#C5562C' }}>{establecimientos.length}</div>
        </div>
        <div style={ds.card}>
      ...
      {!isLoaded ? (
        ...
        >
          ...
          {establecimientos.map(e => (
            e.latitud && e.longitud && (
              <MarkerF
                ...
                icon={{
                  url: `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36"><circle cx="18" cy="18" r="16" fill="#FFFFFF" stroke="#C5562C" stroke-width="2.5"/><text x="18" y="23" text-anchor="middle" font-size="16">${e.tipo === 'restaurante' ? '🍽️' : '🏪'}</text></svg>`)}`,
          ...
          {selected && selected.latitud && (
            ...
            >
              <div style={{ fontFamily: "'Plus Jakarta Sans', system-ui, -apple-system, sans-serif", padding: 4, minWidth: 160 }}>
                <div style={{ fontWeight: 800, fontSize: 13, color: '#1A1815', marginBottom: 2 }}>{selected.nombre}</div>
                <div style={{ fontSize: 11, color: '#6B6356' }}>★ {selected.rating?.toFixed(1)} · {selected.total_resenas} reseñas</div>
